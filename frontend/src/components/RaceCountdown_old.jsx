...
export default function RaceCountdown() {
  ...
  const fetchSeasonProgress = async () => {
    try {
      ...
      console.log("⚠️ No se pudo calcular progreso de temporada");
    }
  };
  ...
  const fetchRealPodium = async (race) => {
    ...
    try {
      // Ergast API usa el formato: http://ergast.com/api/f1/{year}/{round}/results.json
      // Como no tenemos el "round", usamos el nombre del circuito
      // Alternativa: buscar por fecha o ubicación
      
      // Intentar obtener el podio del año y circuito
      ...
      console.log("⚠️ No se pudo obtener podio real, usando datos de ejemplo");
    }

    // Fallback: podio de ejemplo
    setLastPodium([
      { name: "George Russell", position: 1, team: "Mercedes" },
      { name: "Lewis Hamilton", position: 2, team: "Mercedes" },
      { name: "Carlos Sainz", position: 3, team: "Ferrari" },
    ]);
  };
  ...
  if (loading) {
    return (
      <div className="bg-gradient-to-r from-pink-400 to-red-400 rounded-2xl p-8 shadow-xl">
        <div className="animate-pulse">
          <div className="h-8 bg-white bg-opacity-30 rounded w-1/2 mb-4"></div>
          <div className="h-32 bg-white bg-opacity-30 rounded"></div>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="bg-gradient-to-r from-red-400 to-orange-400 rounded-2xl p-8 shadow-xl">
  ...
  if (!nextRace) {
    return (
      <div className="bg-gradient-to-r from-gray-400 to-gray-600 rounded-2xl p-8 shadow-xl">
        <div className="text-center text-white">
  ...
  return (
    <div className="bg-gradient-to-r from-gray-100 via-gray-50 to-white rounded-2xl shadow-2xl overflow-hidden border border-gray-200">
      {/* Header */}
          ...
          <div className="space-y-6">
            ...
            <div>
              <h4 className="text-white text-lg font-semibold mb-4">
                Last Podium at {nextRace?.location || "Circuit"}
              </h4>
              <div className="grid grid-cols-3 gap-4">
                {lastPodium.map((driver, index) => (
                  <div
                    key={index}
                    className={`relative rounded-xl p-4 pt-8 ${
                      driver.position === 1
                        ? "bg-yellow-200 bg-opacity-80 transform -translate-y-2"
                        : driver.position === 2
                        ? "bg-gray-200 bg-opacity-70"
                        : "bg-orange-200 bg-opacity-70"
                    }`}
                  >
                    <div
                      className={`absolute -top-3 left-1/2 transform -translate-x-1/2 w-10 h-10 rounded-full flex items-center justify-center font-bold ${
                        driver.position === 1
                          ? "bg-yellow-400"
                          : driver.position === 2
                          ? "bg-gray-400"
                    ...
                    >
                      {driver.position === 1 ? "1st" : driver.position === 2 ? "2nd" : "3rd"}
                    </div>
                    <p className="text-center text-gray-800 font-semibold text-sm mt-2">
                      {driver.name}
                    </p>
              ...
              <p className="text-xs text-white text-opacity-70 mt-2 text-center">
                * Podio de ejemplo - Próximamente con datos reales
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col justify-center items-center">
            ...
            <div className="grid grid-cols-4 gap-4 mb-8">
              {[
                ...
                <div key={index} className="text-center">
                  <div className="bg-white bg-opacity-90 backdrop-blur-sm rounded-2xl p-6 min-w-[80px] shadow-lg">
                    <div className="text-5xl font-bold text-gray-900 tabular-nums">
                      {String(unit.value).padStart(2, "0")}
                    </div>
                  </div>
                  <div className="text-white text-opacity-90 text-sm font-semibold mt-2">
                    {unit.label}
                  </div>
                </div>
              ))}
            ...
            <div className="w-full">
              <div className="text-white text-center mb-2 font-semibold">
                {Math.round(seasonProgress)}% Completed
              </div>
